...
export function Sidebar({ open, setOpen }) {
    ...
    return (
        <nav className={` h-screen p-2 flex flex-col duration-500 bg-zinc-900 text-zinc-50 border-r border-zinc-400 z-20   ${open ? 'w-60' : 'w-16'}`}>
            ...
            {/* Header */}
            <div className={`px-3 py-2 my-2  cursor-pointer flex justify-evenly items-center relative group`}>
                <div>
                    <Clapperboard strokeWidth={1} size={40} />
                </div>
                <div className={`${!open && 'w-0 -translate-x-4'} duration-500 overflow-hidden`}>SAREGAMA</div>
            </div>
            ...
            <div className={`flex ${open ? 'flex-col items-start gap-2 p-2' : ''}`}>
                ...
                {open && <Button variant="outline" className="text-zinc-600" onClick={() => (setLogoutAlert(!logoutAlert))}>Log Out</Button>}
            </div>
